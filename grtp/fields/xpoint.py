"""
grtp/fields/xpoint.py
=====================
Analytical electromagnetic field configurations for magnetic reconnection
in the Kerr ergosphere.

Two field profiles are implemented, selectable via a `--field-type` CLI flag
or by instantiating the appropriate class directly:

  XPointField      — 2D magnetic null-point (X-point) geometry  [PRIMARY]
  HarrisSheetField — 1D Harris current-sheet profile  [comparison / validation]

Physical construction
---------------------
Both classes follow the same pipeline:

  1. Specify the field in the **local ZAMO orthonormal frame** (Minkowski).
     The ZAMO frame is the natural local rest frame inside the ergosphere and
     ensures that B and E have their physically intuitive meanings.

  2. **Tetrad-transform** to Boyer-Lindquist (BL) coordinate components via:

       F^μν_(BL) = e^μ_(a) e^ν_(b) F^(ab)_(ZAMO)

     where e^μ_(a) is the ZAMO tetrad (see spacetime/kerr.py).

  3. Provide the mixed form F^μ_ν = g_νλ F^μλ for use in the Lorentz EOM.

ZAMO frame EM tensor convention
--------------------------------
Signature (−,+,+,+), frame indices (t̂=0, r̂=1, θ̂=2, φ̂=3):

  F^(t̂)(r̂) =  E^r̂       F^(t̂)(θ̂) =  E^θ̂      F^(t̂)(φ̂) =  E^φ̂
  F^(r̂)(θ̂) =  B^φ̂      F^(r̂)(φ̂) = −B^θ̂     F^(θ̂)(φ̂) =  B^r̂

All components antisymmetric: F^(ab) = −F^(ba).

X-point field physics
---------------------
The 2D magnetic X-point configuration places a magnetic null point at
(r_X, θ_X) in the ZAMO frame:

  B^r̂  =  B₀ · (θ − θ_X) / λ · f(r, θ)    [poloidal field, reverses at θ_X]
  B^θ̂  = −B₀ · (r − r_X) / λ · f(r, θ)    [reversal at r_X → null point]
  E^φ̂  =  E₀ · f(r, θ)                     [reconnecting out-of-plane field]

The Gaussian envelope f(r,θ) = exp(−[(r−r_X)² + (r Δθ)²] / 2λ²) localises
the reconnection region within the current sheet of half-width λ.

The reconnecting E^φ̂ field is the key ingredient: it accelerates particles
along the X-line (toroidal direction), enabling some to attain negative
Killing energy E_∞ < 0 and thereby extract energy from the black hole.

References
----------
  Lyubarsky (2005), MNRAS 358, 113    — analytical X-point reconnection
  Uzdensky (2011), SSRv 160, 45       — Harris-sheet ansatz justification
  Comisso & Asenjo (2021), PRL 127, 111101
  Koide et al. (2000), Science 295, 1688
"""
